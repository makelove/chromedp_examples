...
func main() {
	...
	opts := append(chromedp.DefaultExecAllocatorOptions[:],
		...
		chromedp.Flag("proxy-bypass-list", "<-loopback>"),//跳过
	)
	ctx, cancel := chromedp.NewExecAllocator(context.Background(), opts...)//后台执行 上下文
	defer cancel()
	...
	chromedp.ListenTarget(lctx, func(ev interface{}) {//监听
		switch ev := ev.(type) {
	...
	if err := chromedp.Run(ctx,
		...
		chromedp.Navigate(s.URL),//导航到某URL
	); err != nil {
		log.Fatal(err)
	}

	// to show that further requests (even in new tabs) are authenticated.
	tctx, cancel := chromedp.NewContext(ctx)//新标签也是使用代理
	defer cancel()
...
//新建代理 newProxy creates a proxy that requires authentication.
